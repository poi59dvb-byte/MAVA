...
export class UI {
  ...
  mountBoot(): void {
    this.clear();
    this.layer.innerHTML = `<div class="panel center"><h2>Loading...</h2><p>WASD/Setas para dirigir, Shift drift, Espaço nitro.</p></div>`;
  }
  ...
  hideCountdown(): void {
    this.layer.querySelector('#count')?.remove();
  }
  ...
  mountPause(): HTMLElement {
    const p = document.createElement('div');
    p.className = 'panel center';
    p.id = 'pausePanel';
    p.innerHTML =
      '<h2>Pausado</h2><button id="resume">Continuar</button><button id="pauseRestart">Reiniciar</button><button id="pauseMenu">Menu</button>';
  ...
  showTouch(layout: 'A' | 'B', enabled: boolean): void {
    const a = this.layer.querySelector('#touchA') as HTMLElement | null;
    const b = this.layer.querySelector('#touchB') as HTMLElement | null;
  ...
  updateHud(data: {
    ...
    (this.layer.querySelector('#nitroBar') as HTMLElement).style.width = `${Math.round(data.nitro * 100)}%`;
