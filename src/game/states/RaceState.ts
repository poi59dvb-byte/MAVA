...
export class RaceState implements GameState {
  ...
  update(game: Game, dt: number): void {
    ...
    if (this.countdown > 0) {
      this.countdown -= dt;
      game.ui.showCountdown(this.countdown > 2 ? '3' : this.countdown > 1 ? '2' : this.countdown > 0 ? '1' : 'GO!');
    ...
    if (input.pause && !this.lastPause) {
      game.pausedStateCache = this;
      void game.stateMachine.set(game, new PauseState());
    }
    ...
    if (s.drifting) {
      this.particles.spawn(s.position.x, s.position.z);
      game.engine.audio.drift();
    }
    if (s.nitroActive) game.engine.audio.nitro();

    this.car.sync(s);
    game.engine.cameraRig.follow(s.position, s.heading, s.speed, s.nitroActive);
    game.ui.updateHud({
